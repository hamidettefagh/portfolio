...
import { ProjectRow } from '../../components/patterns/ProjectRow.jsx';
...
if (typeof document !== 'undefined' && !document.getElementById('pf-work-css')) {
  const s = document.createElement('style'); s.id = 'pf-work-css'; s.textContent = WORK_CSS;
  document.head.appendChild(s);
}

export function WorkSection({ detailHref = 'project.html' }) {
  return (
    <section className="pf-section" id="work" data-screen-label="Selected work">
      <div className="pf-section__head">
        <Rule label="Selected work" />
        <h2 className="pf-section__title">Five engagements, one job:<br />make it real</h2>
      </div>
      <div className="pf-work__list">
        {WORK_ITEMS.map((w) => (
          <ProjectRow key={w.index} {...w} href={w.index === '01' ? detailHref : undefined} />
        ))}
      </div>
    </section>
  );
